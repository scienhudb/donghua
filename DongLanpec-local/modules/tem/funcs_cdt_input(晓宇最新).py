...
def _offer_delete_product_when_local_excel_missing(product_id: int, viewer: QWidget, detail: str) -> None:
    ...
    if project_confirm_btn.show_confirm_dialog(
        ...
        if not ok:
            ...
            # 删除成功：给当前条件输入实例打标记，并尝试立即关闭“条件输入”标签页。
            if viewer is not None:
                setattr(viewer, "_product_deleted_due_to_missing_local", True)
                # 如果当前并不在“关闭标签页流程”里，也要主动关掉标签页
                if not getattr(viewer, "_in_close_tab_flow", False) and hasattr(viewer, "_try_close_self_tab_after_local_delete"):
                    try:
                        viewer._try_close_self_tab_after_local_delete()
                    except Exception:
                        pass
