...
def autofill_outer_diameter(viewer: QWidget):
    """
    基于 设计数据表"公称直径*" 与 user_config配置，自动填充"外径系列"和"外径"。
    新逻辑：
    1. 外径系列从user_config的2.2.11.2和2.2.11.3判断（英制系列/公制系列）
    2. 外径值从user_config的2.2.11.1映射表查找
    3. 如果映射表中不存在，使用公式计算（界面显示"—"，但保存计算值）
    """
    ...
    try:
        ...
        # 如果user_config判断失败，尝试从界面读取（兼容旧逻辑）
        if not determined_series:
            determined_series = _get_series_from_general(viewer)
            if not determined_series:
                # 如果都没有，设为"—"并返回
                _set_general_outer_diameter(viewer, "—")
                if hasattr(viewer, "_calculated_outer_diameter"):
        ...
        if guankou_codes:
            ...
            if tube_ca and shell_ca and str(tube_ca) == str(shell_ca):
                for code in guankou_codes:
                    if code:
                        code_to_value[str(code).strip()] = str(tube_ca)
            else:
                ...
                for code in guankou_codes:
                    ...
                    if not c:
                        continue
                    a = query_guankou_affiliation(product_id, c)
                    if a == "管程":
                        code_to_value[c] = str(tube_ca) if tube_ca is not None and str(tube_ca).strip() != "" else ""
                    elif a == "壳程":
                        code_to_value[c] = str(shell_ca) if shell_ca is not None and str(shell_ca).strip() != "" else ""
                    else:
                        code_to_value[c] = ""

            if code_to_value:
                ret = update_guankou_corrosion_to_category_table(product_id, code_to_value)
                print(f"[同步] 管口类别表.接管腐蚀裕量 更新 {ret.get('updated', 0)}/{ret.get('requested', 0)}")
    except Exception as e:
        # 兼容：字段未加/库异常时，不影响主流程
    ...
    try:
        ...
        if viewer is not None:
            ...
            if base_val == "否":
                ...
                # 以配置库为准：外径系列与配置库一致则直接使用，不一致则用配置库并提示（在 validate 中提示）
                viewer._imported_outer_series = data_map.get("外径系列", "").strip()
...
def validate_all_tables_after_import(viewer: QWidget):
    ...
    # 1206新修改-外径：导入时外径系列、外径以配置库为准，若不符则提示
    try:
        if _is_shell_and_tube(viewer) and _is_outer_by_diameter_enabled(viewer):
            imported_series = getattr(viewer, "_imported_outer_series", "")
            determined_series = _determine_diameter_series()
            if imported_series and determined_series and imported_series != determined_series:
                tip_list.append("⚠️ 导入外径系列与配置库不符，当前已设置为以配置库为主")
    except Exception:
        pass
