...
class DesignParameterDefineInputerViewer(QWidget):
    ...
    def toggle_batch_replace_row(self, row):
            ...
            for r in range_rows:
                ...
                if not r_eid:
                    try:
                        ...
                        r_eid = None
                if r_eid and r_eid not in target_set:
                    targets.append(r_eid)
                    target_set.add(r_eid)
        else:
        ...
        self.refresh_batch_replace_row_highlight(changed_rows)
    ...
    def refresh_batch_replace_row_highlight(self, rows=None):
        ...
        try:
            for r in rows_to_refresh:
                ...
                color = QColor("#d0e7ff") if eid in targets and not is_gasket else QColor(
                    "#ffffff" if r % 2 == 0 else "#f6f6f6"
                )
    ...
    def eventFilter(self, obj, event):
        ...
        try:
            ...
            if (
                (obj is parts_table or obj is parts_viewport)
    ...
    def enter_batch_replace_mode(self):
        ...
        # 进入批量替换模式时断开普通 itemClicked 逻辑。
        # 批量选择只走现有的 cellClicked -> on_left_table_cell_clicked，
        # 避免一次点击被 cellClicked 和 itemClicked 重复处理。
        try:
            self.tableWidget_parts.itemClicked.disconnect()
        except Exception:
            pass
