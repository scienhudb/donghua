...
def set_pipe_function_column_readonly(stats_widget):
    ...
    if product_type == "管壳式热交换器":
        if product_version in ["AEU", "BEU"]:
            ...
            readonly_values = {"管程入口", "管程出口", "壳程入口", "壳程气相出口","壳程液相出口","壳程液位计接管1","壳程液位计接管2","壳程温度计接管"}
    ...
    if product_type == "管壳式热交换器" and product_version in ["NEN", "BEM","AES","BES","AEU","BEU","AEM"]:
        ...
        belong_editable_functions ={"管程入口", "管程出口", "壳程入口","壳程液位计接管1","壳程液位计接管2","壳程温度计接管"}
    ...
    if source_code_item:
        ...
        for col_idx, field_name in enumerate(fields, start=1):  # 从第1列开始（跳过序号列）
            ...
            if source_item:
                ...
                if col_idx == 2 and text_to_set.strip() in {"管程出口", "管程入口", "壳程出口", "壳程入口","排气口","排液口","壳程气相出口","壳程液相出口","壳程液位计接管1","壳程液位计接管2","壳程温度计接管"}:
                    text_to_set = ""
...
def set_default_pipe_cannot_be_deleted(stats_widget):
    ...
    if product_type == "管壳式热交换器":
        if product_version in ["AEU", "BEU"]:
            ...
            readonly_pipe_functions = {"管程入口", "管程出口", "壳程入口", "壳程气相出口","壳程液相出口","壳程液位计接管1","壳程液位计接管2","壳程温度计接管"}
