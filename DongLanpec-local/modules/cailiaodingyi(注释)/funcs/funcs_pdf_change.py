...
_DESIGN_ROWS_CACHE = {}  # 缓存设计数据行
_GASKET_MAPPING_CACHE = {}  # 垫片-法兰映射缓存（单条）
_GASKET_MAPPINGS_ALL_CACHE = {}  # 垫片-法兰映射缓存（全量）
_MAP_GTYPE_CACHE = {}  # 垫片类型->代号缓存
_GASKET_DIM_CACHE = {}  # 垫片尺寸查询缓存
_FLANGE_MATERIAL_CACHE = {}  # 法兰材料牌号缓存
_COMPUTE_PN_CACHE = {}  # 垫片推荐PN计算结果缓存
_PRODUCT_FORM_CACHE = {}  # 产品型式缓存
...
def insert_or_update_element_data(element_original_info, product_id, template_name):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # 如果找到该产品ID的模板名称的记录则保留（说明已经是当前模板，无需操作）
            if result['COUNT(*)'] > 0:
                return

            # 如果没找到该产品ID的模板名称的记录，先删除原模板对应的产品零件信息（清空旧数据）
            if result['COUNT(*)'] == 0:
...
def update_param_table_data(table: QTableWidget, product_id: int, element_id: int):
    ...
    # 辅助函数：获取表格中某行某列的值，支持QComboBox、QLineEdit和普通QTableWidgetItem
...
def toggle_covering_fields(table, combo, control_field):
    ...
    is_covering = combo.currentText() == "是"  # 判断是否添加覆层

    # 遍历表格，根据是否添加覆层来控制相关行的显示/隐藏
    for row in range(table.rowCount()):
...
def is_all_guankou_parts_defined(product_id: int) -> bool:
    """
    最终版：综合管口定义表 + 管口参数表完整性校验，判断所有管口零件是否已定义。
    """
    覆层相关字段 = [
        "覆层材料类型", "覆层材料牌号", "覆层材料级别",
        "覆层材料标准", "覆层成型工艺", "覆层使用状态", "覆层厚度"
...
def get_filtered_material_options(selected: dict) -> dict:
    """根据当前已选字段，查询数据库，返回所有材料字段的可选项（用于级联下拉）"""
    material_fields = ['材料类型', '材料牌号', '材料标准', '供货状态']
    # 动态构建WHERE条件：只筛选已选中的字段
...
def query_image_from_database(template_name, element_id, has_covering):
    """从模板库中查询元件示意图路径（根据是否有覆层选择不同字段）"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            # 如果有覆层则取元件示意图覆层字段，否则取元件示意图字段
...
# 注意：下面重复定义了 query_guankou_image_from_database，原代码如此，保留不动
def query_guankou_image_from_database(template_id, guankou_id, has_covering):
    # 从管口零件表中查询图片信息
    connection = get_connection(**db_config_2)
    try:
...
def get_dependency_mapping_from_db():
    ...
    """
    # 导入必要的模块：json用于解析JSON格式的联动选项，re用于正则分割
    import json, re
    # 连接材料库（db_config_2）
    conn = get_connection(**db_config_2)
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 辅助函数：将联动选项安全转换为列表，支持JSON数组或普通分隔符
            def _to_list(s):
            ...
            for r in rows1:
                ...
                # 合并选项（因为同一主参数+主值+从参数可能有多行，比如不同元件）
...
def toggle_dependent_fields_complex(table, conditions: dict, target_fields: list):
    ...
    try:
        ...
        # 第二遍遍历：根据 satisfied 状态设置目标字段的显隐
        for row in range(table.rowCount()):
            param_item = table.item(row, 0)
...
def query_param_by_component_id(component_id, product_id):
    """
    根据元件ID和产品ID，从产品设计活动库的元件附加参数表中查询所有参数。
    返回一个字典：{参数名称: 参数值}
    """
    connection = get_connection(**db_config_1)
    try:
...
def sync_yanban_height_if_exceeds_shell_dn(product_id) -> None:
    ...
    a = ctx["shell_dn"]          # 壳程公称直径
    auto_h = ctx["auto_h"]       # 自动推荐高度
    overflow = bool(ctx["overflow"])  # 是否超出
    conn = None
    try:
        conn = get_connection(**db_config_1)
        # 查询当前产品中所有堰板元件的高度参数
...
def update_element_para_data(product_id, element_name, param_name, param_value):
    ...
    注意：这里参数名为元件名称，实际应当是元件ID，函数名有误导，但保持原样。
    """
    conn = get_connection(**db_config_1)
    try:
        with conn.cursor() as cursor:
    ...
    try:
        with conn.cursor() as c:
            ...
            if selected_codes:
                ...
                """
                # 执行占用：将本次选中的管口代号的材料分类设置为当前 tab 标签
...
def save_guankou_codes_for_tab(product_id, category_label, selected_codes):
    """
    保存某个 tab（管口类别）下选中的管口代号。
    先清空该类别下所有管口，再重新占用选中的管口。
    """
    # 连接产品设计活动库
    conn = pymysql.connect(**db_config_1)
    try:
        with conn.cursor() as c:
            # 释放本 tab 之前占用的所有管口（将材料分类置为 NULL）
            c.execute("""
                UPDATE 产品设计活动表_管口类别表
                SET 材料分类 = NULL
                WHERE 产品ID = %s AND 材料分类 = %s
...
def query_template_codes(product_id):
    """
    查询产品中所有管口的信息（管口ID、管口代号、管口所属元件）。
    返回结果列表，每个元素为字典（键为字段名）。
    """
    # 连接产品设计活动库
    connection = get_connection(**db_config_1)
    try:
...
def update_guankou_params_bulk(rows: Iterable[Tuple[str, str, str, Any]],
    ...
    # 连接数据库（使用原生 pymysql，因为前面用了 pymysql.connect）
    conn = pymysql.connect(**db_config_1)
    updated = 0
    missing: List[Tuple[str, str, str]] = []
    try:
        with conn.cursor() as c:
            # 更新 SQL：按产品ID、类别、参数名称匹配，更新参数值
            sql = """
                UPDATE `产品设计活动表_管口附加参数表`
                SET `参数值`=%s
...
def get_numeric_rules() -> Tuple[
    ...
    """
    从参数校验规则表中读取数值规则，返回四部分：
      - gt0_set: 要求 >0 的参数名集合
      - ge0_set: 要求 >=0 的参数名集合
      - range_map: 范围规则 {参数名: (最小值, 最大值, 含下限, 含上限)}
      - allowed_map: 允许的字面值集合 {参数名: {允许值1, 允许值2, ...}}
    """
    # 初始化返回容器
    gt0_set: Set[str] = set()
    ge0_set: Set[str] = set()
    ...
    # 规范化规则类型字符串，统一为 gt0 / ge0 / range 等
    ...
    try:
        ...
        for row in rows:
            ...
            # 处理允许字面值：按逗号分割，去重后加入 allowed_map
            if allow_txt:
...
def clear_guankou_category(product_id, category_label):
    """
    清空某个产品在某个管口类别下的管口ID
    即：将指定产品、指定材料分类的所有记录的“材料分类”字段置为 NULL。
    """
    # 连接产品库
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            # 执行更新，将匹配条件的行的材料分类设为 NULL
            cursor.execute("""
                UPDATE 产品设计活动表_管口类别表
                SET 材料分类 = NULL
                WHERE 产品ID=%s AND 材料分类=%s
...
def _cell_text(t: QTableWidget, r: int, c: int) -> str:
    """
    从表格的指定单元格获取文本。
    支持 QComboBox 和 QLineEdit 等控件，否则取 QTableWidgetItem。
    """
    w = t.cellWidget(r, c)
    if isinstance(w, QComboBox):
        return (w.currentText() or "").strip()
...
def query_guankou_affiliation(product_id, guankou_code):
    """
    安全查询管口归属（管程/壳程）。
    根据管口所属元件字段的值推断：包含“管”字则为管程，包含“壳”或“外头盖”则为壳程。
    """
    affiliation = None
    try:
        # 每次都新开连接，避免连接池问题
...
def _first_nonempty(*vals):
    """
    返回第一个非空（不是 None 也不是空字符串）的值，去除首尾空格。
    若全部为空则返回空字符串。
    """
    for v in vals:
        if v not in (None, ""):
...
def _normalize_dn(s):
    """
    标准化公称直径：如果值是整数（如 800.0），则转为整数字符串 "800"；否则返回原字符串。
    """
    if not s: return ""
    try:
        f = float(s)
...
def _normalize_product_form_list(forms_text: str) -> list:
    """
    将产品型式字符串（可能包含各种分隔符）规范化为列表。
    参数:
        forms_text: 原始产品型式文本，如 "A,B;C;D"
    返回:
        去除空格并转换为大写后的列表，如 ["A","B","C","D"]
    """
    # 将输入转为字符串，去除首尾空格，转为大写
    s = str(forms_text or "").strip().upper()
    # 定义常见中文/英文分隔符，统一替换为英文逗号
    for sep in ("，", "、", ";", "；", "/", "\\"):
        s = s.replace(sep, ",")
    # 按逗号拆分，去除每个部分的空白，过滤掉空字符串
...
def _is_form_match(forms_text: str, product_form: str) -> bool:
    ...
    判断给定的产品型式是否匹配产品型式列表。
    参数:
        forms_text: 产品型式列表字符串（支持分隔符）
        product_form: 当前产品的型式
    返回:
        匹配返回 True，否则 False
    """
    # 规范化型式列表为 tokens
...
def _is_form_all(forms_text: str) -> bool:
    """
    判断产品型式列表中是否包含 "ALL" 通配符。
    参数:
        forms_text: 产品型式列表字符串
    返回:
        如果包含 "ALL" 返回 True，空列表也返回 True（视为通配所有），否则 False
    """
    # 规范化型式列表
    tokens = _normalize_product_form_list(forms_text)
    # 空列表视为通配所有
...
def get_gasket_mapping(gasket_name: str, product_id: str = "") -> dict:
    ...
    try:
        with conn.cursor() as cur:
            ...
            for row in rows:
                forms = row.get("产品型式")
                # 如果产品型式完全匹配当前产品型式，加入preferred
                if _is_form_match(forms, product_form):
                    preferred.append(row)
                # 如果包含ALL通配，加入fallback
                elif _is_form_all(forms):
                    fallback.append(row)

            # 优先取preferred的第一条，否则取fallback的第一条
...
def get_gasket_mappings_all(gasket_name: str, product_id: str = "") -> list:
    """
    获取指定垫片名称对应的所有匹配的配套法兰信息（不考虑优先级，返回列表）。
    与 get_gasket_mapping 类似，但返回所有匹配项（精确匹配优先，否则ALL匹配）。
    """
    # 初始化结果列表
    res = []
    if not gasket_name:
...
def query_element_name_param_value(product_id: str, element_name: str, param_name: str):
    """
    根据产品ID、元件名称、参数名称查询参数值。
    返回查询到的参数值（字符串），若未找到返回None。
    """
    conn = get_connection(**db_config_1)
    try:
        with conn.cursor() as cur:
...
def invalidate_caches_for_product(product_id: str):
    """
    当产品数据更新时，使与该产品相关的缓存失效。
    清除的缓存包括：设计行数据缓存、PN计算缓存、法兰材料缓存、
    垫片映射缓存、产品型式缓存等。
    """
    try:
...
def compute_pn_for_gasket(product_id: str, gasket_name: str):
    ...
    算法：
        1. 获取管程和壳程的设计压力、设计温度。
        2. 获取该垫片所有可能的法兰映射配置。
        3. 对每种配置，根据法兰侧别、法兰材料、温度压力计算对应的PN值。
        4. 如果垫片为“平盖垫片”，优先取“管箱法兰”对应的PN，否则取首个匹配的PN。
        5. 其他垫片取所有计算结果的最大PN。
    """
...
def _get_flange_material_by_name(product_id: str, flange_name: str) -> str:
    """
    根据产品ID和法兰名称获取法兰的材料牌号。
    先从元件材料表中找到元件ID，再从附加参数表中找"材料牌号"参数。
    """
    if not flange_name:
        return ""
...
def _compute_pn_inline(material: str, T: str, P: str):
    """
    根据法兰材料、设计温度T、设计压力P，从压力等级表中插值计算所需的公称压力PN。
    算法：
        1. 从压力等级表中查询材料对应的温度-压力数据。
        2. 如果表中直接有对应温度的压力值，则直接取用。
        3. 否则，根据最近的两个温度点线性插值得到该温度下的许用压力。
        4. 取满足 P <= 许用压力的最小PN等级。
    """
    try:
        if not material:
    ...
    candidate = None
    candidate_row = None
    # 遍历每一行（通常每种材料对应多行PN等级）
    for row in rows:
        px = _get_col(row, Tf)
        if px is None:
            # 线性插值：找到低于和高于目标温度的两个最近温度点
...
def get_fastener_bolt_type_options():
    """
    获取设备法兰紧固件中“螺柱型式”的候选项。
    选项来源：参数表中参数名称='螺柱型式'的JSON数组参数值。
    """
