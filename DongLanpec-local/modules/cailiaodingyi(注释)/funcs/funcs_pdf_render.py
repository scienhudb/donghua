...
class _CopyPasteEventFilter(QtCore.QObject):
    """
    自定义事件过滤器，专门处理表格的复制/粘贴快捷键。
    将事件拦截后，调用自定义的复制/粘贴逻辑，绕过默认的 Qt 行为。
    """
    ...
    def eventFilter(self, obj, ev):
        """拦截键盘事件，如果是 Ctrl+C 或 Ctrl+V 则执行自定义操作并返回 True 表示已处理"""
        if ev.type() == QEvent.KeyPress:
            if ev.matches(QKeySequence.Copy):
                _copy_cells(self.table)                 # 调用自定义复制函数
                ev.accept()
                return True
            if ev.matches(QKeySequence.Paste):
                _paste_cells(self.table, self.groups, self.row2field, self.row2group)  # 调用自定义粘贴函数
                ev.accept()
                return True
        return False
...
def install_copy_paste_shortcuts(table: QTableWidget, groups, row2field, row2group):
    """
    安装表格的复制/粘贴快捷键和事件过滤器，确保在表格及其子控件上都能使用 Ctrl+C/V。
    """
    # 1) 创建全局快捷键，作用于表格及其所有子控件
    ...
    sc_copy.activated.connect(lambda: _copy_cells(table))           # 触发自定义复制
    sc_paste.activated.connect(lambda: _paste_cells(table, groups, row2field, row2group))  # 触发自定义粘贴

    # 2) 安装事件过滤器作为兜底，防止焦点在某些子控件上时快捷键失效
    filt = _CopyPasteEventFilter(table, groups, row2field, row2group)
    table._copy_paste_filter = filt       # 保持引用避免被垃圾回收
    table.installEventFilter(filt)        # 给表格安装过滤器
    table.viewport().installEventFilter(filt)  # 给视口也安装，确保视口事件能被捕获

def group_guankou_params_by_prefix(guankou_para_info: list) -> dict:
    """
    将管口参数数据按参数名前缀分组，处理“单列字段”和“多列字段(带1/2/3后缀)”。
    返回一个字典，单列字段直接映射到值，多列字段映射到一个 {列号: 值} 的子字典。
    """
    result = {}
    multi_col_fields = defaultdict(dict)  # 用于暂存多列字段：{基础名: {列号: 值}}
    single_col_fields = {}                # 用于暂存单列字段

    for item in guankou_para_info:
        raw_name = str(item.get("参数名称", "")).strip()
        # ✅ 兼容 “参数值” 和 “参数数值” 两个不同的字段名
        value = item.get("参数值") or item.get("参数数值") or ""

        # 用正则匹配参数名末尾的数字（1~3），例如 "接管材料类型1"
    ...
    result.update(single_col_fields)      # 先加入单列字段
    result.update(multi_col_fields)       # 再加入多列字段（以基础名作为key）
    return result
...
class FreezeUI:
    """
    上下文管理器，用于批量修改表格时暂时关闭信号和界面刷新，最后一次性恢复。
    可以传入多个控件同时冻结。
    """
    ...
    def __enter__(self):
        """进入上下文时，记录每个控件的原状态，然后禁用信号和刷新"""
        for w in self.widgets:
            self._states.append((w, w.signalsBlocked(), w.updatesEnabled()))
            w.blockSignals(True)       # 阻塞信号
            w.setUpdatesEnabled(False)  # 禁止界面更新
        return self

    def __exit__(self, *args):
        """退出上下文时，恢复之前的状态，并强制刷新一次视口"""
        for w, sb, ue in self._states:
            w.blockSignals(sb)         # 恢复信号阻塞状态
            w.setUpdatesEnabled(ue)    # 恢复更新状态
            try:
                w.viewport().update()  # 强制重绘视口
            except Exception:
                pass
...
def find_material_groups_fuzzy_strict(table):
    ...
    # 文本规范化：去除空白、数字、括号内容，方便关键词匹配
    def norm(s: str) -> str:
        if not s: return ''
        s = s.strip()
        s = re.sub(r'\s+', '', s)                # 去除所有空白字符
        s = re.sub(r'[0-9０-９]+', '', s)         # 去除半角/全角数字
        s = re.sub(r'（.*?）|\(.*?\)', '', s)     # 去除括号及其内容
        return s

    def hit_key(txt: str):
        """判断规范化后的文本是否包含某个材料字段关键词，返回匹配到的字段名"""
        for k in KEYS:
            if k in txt:
                return k
        return None
    ...
    def maybe_commit(cur_map):
        # 只有满四项才认为是一个完整的材料组，将其保存
        if len(cur_map) == 4:
            gi = len(groups)
            groups.append(cur_map.copy())
            # 建立 行号->字段 和 行号->组索引 的映射
    ...
    for r in range(table.rowCount()):
        it = table.item(r, 0)  # 第一列是参数名称
        txt = norm(it.text() if it else '')
        k = hit_key(txt)

        if not k:
            # 遇到非材料字段行：尝试提交当前组，然后清空缓存重新开始
            maybe_commit(cur)
            cur.clear()
            continue
...
def ensure_editable_item(table: QTableWidget, r: int, c: int) -> bool:
    """保证目标单元格存在 QTableWidgetItem 且具有可编辑标志；如果没有则创建并设置标志"""
    it = table.item(r, c)
    if it is None:
        it = QTableWidgetItem("")
...
def _apply_material_paste_batch(table, col: int, rows_map: dict, new_vals: dict):
    ...
    # 4) 如果材料类型和材料牌号都已有值，尝试自动填充材料标准和供货状态（当候选唯一时）
...
def _copy_cells(table: QTableWidget):
    ...
    # 如果选区矩形是 1x1，但可能用户通过 Ctrl 点选了多个离散单元格，则扩展为外接矩形
...
def _paste_cells(table: QTableWidget, groups, row2field, row2group):
    """粘贴逻辑：从剪贴板读取文本，按 TSV 格式解析并写入表格对应位置，然后触发材料联动"""
    ...
    # 单值且有选区时，将该值填充到整个选区；否则按网格大小覆盖
    fill_mode = (rows_in == 1 and cols_in == 1 and bool(rngs))

    touched = set()  # 记录实际写入的单元格坐标
    ...
    # —— 粘贴后：按【(组,列)】分桶，严格顺序触发材料联动 —— #
    order = ('材料类型', '材料牌号', '材料标准', '供货状态')

    buckets = {}  # key: (组索引, 列) -> value: {字段名: 最终值}
    for (r, c) in touched:
        gi = row2group.get(r)
    ...
    # 对每个 (组,列) 批量应用粘贴后的联动，避免“类型强清”破坏已粘贴值
...
def install_reinforcement_group_toggle(
    ...
    def _get_text(r, c):
        """获取单元格文本（兼容 QComboBox 和 QLineEdit）"""
    ...
    def _refresh():
        ...
        table.viewport().update()  # 刷新界面

    # 初始化时应用一次
    _refresh()

    # 连接开关字段的变化事件（如果是 QComboBox）
    if toggle_row >= 0:
        ...
        if isinstance(wdg, QComboBox):
            ...
            try:
                wdg.currentTextChanged.disconnect()   # 断开旧连接
            except Exception:
                pass
            try:
    ...
    table._reinforcement_toggle_conn = _on_data_changed  # 保存引用
...
def install_overlay_group_toggle(
    ...
    *,
    ...
    """
    安装覆层字段组的联动显示/隐藏逻辑：
    - 控制覆层开关（是否添加覆层）决定整组可见性
    - 根据材料类型（钢板/焊材）动态禁用级别、使用状态、成型工艺等行
    - 为工艺行安装按列区分候选的委托
    """
    if not table or table.rowCount() == 0:
        return
    ...
    for g in groups:
        ...
        for r in range(table.rowCount()):
            ...
            if any(pname.startswith(px) for px in prefixes):
                overlay_rows.append(r)   # 属于覆层字段的行
...
def render_guankou_param_to_ui(viewer_instance, guankou_para_info: list):
    """
    将管口参数数据渲染到当前激活的管口参数表格中。
    包含：表格创建、多列结构解析、下拉代理安装、数值输入委托、复制粘贴、补强圈/覆层联动等。
    """
    table = viewer_instance.tableWidget_guankou  # 当前激活的管口表格

    # === 渲染前调试信息：打印当前tab和数据条数 ===
    try:
    ...
    table.clear()
    table.setRowCount(0)
    table.setColumnCount(4)                     # 4列：参数名 + 三列值
    table.verticalHeader().setVisible(False)    # 隐藏垂直表头（行号）
    table.horizontalHeader().setVisible(False)  # 隐藏水平表头
    table.setSelectionBehavior(QTableWidget.SelectItems)  # 以单元格为单位选择
    table.setSelectionMode(QTableWidget.ExtendedSelection)  # 支持扩展选择（Ctrl/Shift）
    install_selection_debug(table)              # 安装调试工具（开发环境用）
    table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # 初始禁止编辑，后续通过点击激活
    table.setProperty("user_edited_corrosion", False)       # 记录用户是否编辑过腐蚀裕量
    table.setProperty("user_edited_opening_weld_joint_coeff", False)  # 记录是否编辑过焊接接头系数

    # 安装下拉框的弹出事件过滤器，用于优化下拉菜单的交互
    ...
    class NumericDelegate(QStyledItemDelegate):
        def __init__(self, rule: str, pname: str, table=None, viewer=None, broadcast_row=True):
            ...
            self._targets_cache = []  # 缓存的同行选中列

        def _snapshot_targets(self, row: int):
            """在进入编辑前，记录同一行中所有可编辑的列（用于批量填充）"""
            cols = []
        ...
        def createEditor(self, parent, option, index):
            """创建编辑器（QLineEdit），同时快照同行可编辑列以备批量填充"""
        ...
        def _restore_item_text(self, model, index, text):
            """将文本写回 item（绕过 model.setData 可能的重绘问题）"""
            if not self.table:
                return
        ...
        def setModelData(self, editor, model, index):
            ...
            # 针对“所属元件开孔处焊接接头系数”：不能小于默认值 D（存储在 UserRole+1 中）
    ...
    header.setSectionResizeMode(0, QHeaderView.ResizeToContents)  # 参数名列自适应内容
    for col in range(1, 4):
        header.setSectionResizeMode(col, QHeaderView.Stretch)  # 值列拉伸填满
    ...
    # 确保这些材料行可编辑，并移除可能已经存在的 cellWidget
    for r in found_rows:
        for c in (1, 2, 3):
            it = table.item(r, c)
            if it is None:
...
def render_fastener_param_to_ui(viewer_instance, fastener_para_info: list):
    ...
    for idx, (pno_label, data) in enumerate(sorted_tab_items):
        # 计算当前 tab 应显示的“元件所属”候选（排除其他 tab 已选的）
        already_used = set()
...
def _render_fastener_table_data(table, data, param_structures, dropdown_options, max_cols, viewer_instance=None):
    ...
    table.clear()
    table.setRowCount(0)
    table.setColumnCount(max_cols)             # 设置列数
