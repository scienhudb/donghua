...
def has_product(product_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            return result['COUNT(*)'] > 0  # 只要有记录就返回True
...
def query_all_guankou_categories(product_id):
    """
    查询初始加载活动库里的多个类别。
    优先按Tab_ID排序返回去重后的类别列表。
    如果没有找到带Tab_ID的记录，则尝试用旧数据（仅类别），并为其自动生成Tab_ID后更新数据库。
    返回: 类别列表（为了保持向后兼容）
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            # 优先查询有Tab_ID的类别，按Tab_ID排序保证顺序稳定
            sql = """
            ...
            # 如果没有Tab_ID，尝试兼容旧数据：只返回类别列表，并生成Tab_ID写入数据库
            if not categories:
                sql_old = """
                    SELECT DISTINCT 类别 
                    FROM 产品设计活动表_管口附加参数表 
...
def query_all_guankou_categories_with_tab_id(product_id):
    ...
    只返回Tab_ID非空的记录，用于构建精确的类别与Tab_ID映射。
    返回: {类别: Tab_ID} 的字典
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(sql, (product_id,))
            result = cursor.fetchall()
            # 返回 {类别: Tab_ID} 的字典，方便根据类别快速找到Tab_ID
...
def move_guankou_to_first(element_list):
    """
    将零件名称为'管口'的元素移动到列表第一行。
    遍历列表找到管口，将其弹出并插入到索引0位置。
    """
...
def load_guankou_define_data(product_type, product_form, template_id):
    """根据产品类型、产品形式、模板ID查询管口零件材料表，获取管口定义信息"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def insert_element_data(element_original_info, product_id, template_name):
    """
    将元件数据插入到产品设计活动库的元件材料表中。
    如果该产品ID已有数据则跳过，防止重复初始化。
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            # 先查看是否存在该产品ID的数据，存在则跳过插入
...
def insert_guankou_material_data(material_info, product_id, template_name):
    """
    将管口材料定义数据插入到产品设计活动库的管口零件材料表中。
    同时插入产品ID和模板名称，类别默认为"管口材料分类1"。
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            # 先查看是否存在该产品ID对应的数据，有则跳过
...
def query_template_guankou_para_data(template_id):
    """根据模板ID查询材料库的管口附加参数表，获取该模板的所有管口参数"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def insert_guankou_para_data(product_id, guankou_para_info, template_name, template_id=None):
    ...
    为每个分类生成唯一的Tab_ID，并确保至少有两个基础分类（管口材料分类-管程 和 管口材料分类-壳程）。

    Args:
        product_id: 产品ID
        guankou_para_info: 从模板库查询的管口参数数据
        template_name: 模板名称
        template_id: 模板ID（可选，用于查询"管口材料分类-壳程"的数据）
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            ...
            for item in guankou_para_info:
                ...
                cursor.execute(sql, (
                    ...
                    item['参数单位'],
                    category,
                    category_tab_map[category],  # 使用生成的Tab_ID
                    template_name
                ))
                            ...
                            if category2_items:
                                ...
                                for item in category2_items:
                                    ...
                                    cursor.execute(sql, (
                                        ...
                                        item['参数单位'],
                                        '管口材料分类-壳程',
                                        category_tab_map['管口材料分类-壳程'],  # 使用已生成的Tab_ID
                                        template_name
                                    ))
...
def query_template_element_para_data(template_id):
    """根据模板ID查询材料库的元件附加参数表，获取模板所有元件的附加参数"""
    connection = get_connection(**db_config_2)
    # print("查询元件附加参数列表")
    try:
        with connection.cursor() as cursor:
...
def insert_add_guankou_define(guankou_define_data, category_label, product_id, select_template, tab_id=None):
    """
    将新增的管口材料定义写入活动库。
    如果tab_id为None，会生成新的Tab_ID用于标识该分类。
    仅当数据库中存在匹配的产品ID和模板名称时才执行插入。
    """
    connection = pymysql.connect(**db_config_1)
    try:
        with connection.cursor() as cursor:
            # 检查是否存在匹配的产品ID和模板名称，不存在则跳过
            check_sql = """
...
def query_guankou_define_data_by_category(product_id, category_or_tab_id):
    """
    查询活动库里的管口定义信息。
    优先通过Tab_ID查询，如果没有结果则尝试通过类别查询（兼容旧数据）。
    """
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
...
def query_guankou_param_by_product(product_id, category_or_tab_id):
    """
    根据产品ID和Tab_ID（或类别）从产品设计活动库中读取管口零件参数数据。
    优先通过Tab_ID查询，如果没有结果则尝试通过类别查询（兼容旧数据）。
    """
    connection = get_connection(**db_config_1)
    try:
...
def query_guankou_param_by_template(category):
    """根据类别从材料库的管口附加参数表中读取参数数据（用于模板库查询）"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def update_template_input_editable_state(self):
    """
    根据左侧表格所有行的定义状态，控制“存为模板”输入框是否可编辑。
    只有全部已定义时才允许输入模板名称。
    """
    if is_all_defined_in_left_table(self.tableWidget_parts, define_status_col=7):  # 假设第7列是定义状态
        ...
        self.lineEdit_template.clear()  # 禁止时清空内容
...
def save_to_template_library(template_name, product_data, product_type, product_form):
    """
    将当前产品定义好的信息存入模板库中。
    如果模板名称已存在，则沿用现有模板ID；否则从5000起分配新ID。
    逐条插入元件材料模板表。
    """
...
def insert_updated_element_para_data(template_id, updated_element_para):
    """将从活动库的元件附加参数表读出的数据写入材料库中的元件附加参数表（用于模板更新）"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
...
def insert_guankou_define_data(template_id, updated_guankou_define):
    """将从活动库的管口定义表读出的数据写入材料库中的管口附加参数表（用于模板更新）"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
...
def load_guankou_material_detail_template(element_id, first_template_id):
    """根据管口零件ID和模板ID查询材料库中的管口零件材料参数详细列表"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def get_grouped(product_id):
    """根据产品ID查询对应的管口分类和管口代号，用于统计各分类下的管口"""
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def get_options_for_param(param_name):
    """
    根据参数名称从材料库的参数表中获取对应的选项列表。
    某些纯数值参数（如焊缝截面积等）不读取下拉选项。
    """
...
def insert_guankou_param_leibie(product_id, category_label, template_name, guankou_para_info, keep_values=True, tab_id=None):
    ...
    try:
        with conn.cursor() as cur:
            # 如果没有提供tab_id，尝试从现有数据中获取，否则生成新的
            if tab_id is None:
                # 先查询该类别是否已有Tab_ID
                cur.execute("""
            ...
            # 使用 ON DUPLICATE KEY UPDATE 避免重复键冲突
            sql = """
                INSERT INTO 产品设计活动表_管口附加参数表
...
def load_guankou_param_byid(category_label, product_id, select_template, guankou_param_id):
    """根据管口零件ID、类别等条件从活动库的管口零件材料参数表查询参数数据"""
    connection = get_connection(**db_config_1)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def is_flatcover_trim_param_applicable(product_id: str) -> bool:
    """判断当前产品是否为管壳式热交换器且型式为AES/AEU，用于平盖修边参数适用性检查"""
    try:
        connection = get_connection(**db_config_1)
        with connection.cursor() as cursor:
...
def delete_guankou_data_from_db(product_id, tab_name):
    """
    删除产品ID对应某个分类（由Tab_ID或类别名标识）的所有管口附加参数数据。
    优先通过Tab_ID删除，若无则尝试通过类别名删除（兼容旧数据）。
    """
    try:
        connection = get_connection(**db_config_1)
        with connection.cursor() as cursor:
...
def load_dropdown_options() -> dict:
    """
    从材料库的参数表读取所有参数的下拉选项配置。
    返回一个字典：{参数名称: [选项列表]}
    注意：选项列表最前面会插入一个空字符串，用于支持清空选择。
    """
    connection = pymysql.connect(**db_config_2)
    try:
        with connection.cursor() as cursor:
            ...
            for name, val in rows:
                try:
                    items = json.loads(val)  # 参数值存储为JSON
                    if "" not in items:
                        items.insert(0, "")  # 插入空选项
...
def _component_to_material_category(component):
    """
    根据管口所属元件的描述判断材料分类。
    包含“管程”或“管箱”→管口材料分类-管程；
    包含“壳程”、“壳体”、“外头盖”→管口材料分类-壳程。
    """
    s = str(component or '')
        ...
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            try:
                ...
                for row in guankou_rows:
                    ...
                    if not code:
                        continue
                    cat = _component_to_material_category(comp)  # 根据元件名判断分类
                    if cat:
            ...
            for item in guankou_info:
                ...
                material_category = guankou_code_to_category.get(code)  # 获取对应的分类
...
def query_unassigned_codes(product_id):
    """
    查询当前产品中尚未分配材料分类（材料分类 IS NULL）的管口代号。
    这些管口可用于自由分配。
    """
...
def _find_row(table, label_text: str):
    """在表格的第一列中查找文本与 label_text 匹配的行索引，找不到返回 None"""
...
def query_template_element_merged_para_data(template_id, element_id):
    """从材料库查询元件附加参数合并表模板数据，按 Tab分类、参数名称排序"""
    connection = get_connection(**db_config_2)
    try:
        with connection.cursor() as cursor:
            sql = """
...
def get_fastener_param_structure_from_db() -> list:
    """从材料库读取设备法兰紧固件的参数结构配置，返回列名与原始数据一致的元组列表"""
    connection = pymysql.connect(**db_config_2)
    try:
        with connection.cursor() as cursor:
