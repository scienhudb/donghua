...
"""
# 导入必要的标准库和第三方库
from collections import defaultdict  # 用于按标题分组时自动创建列表
...
def _set_tip(viewer_instance, msg, *, success=True, auto_clear_ms=5000):
    ...
    try:
        tip = getattr(viewer_instance, "line_tip", None)  # 获取界面左下角的提示标签
        if not tip:
            return
        # 根据成功/失败设置文字颜色：成功黑色，失败红色
...
def get_attachment_tab_types_by_product(product_id):
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            fixed_tab_order = ["接管法兰配对法兰", "接管拉筋", "防冲挡板", "破涡器"]  # 预定义的显示顺序
...
def copy_attachment_tab_from_current(product_id, attachment_type, source_tab_name, source_param_values, viewer_instance):
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            # 确定新tab的名称，如果已存在“接管法兰配对法兰”，则新名称为“接管法兰配对法兰2”，以此类推
            ...
            # 3. 生成新的Tab_ID（使用时间戳确保唯一）
            new_tab_id = int(time.time() * 1000)

            # 4. 获取当前最大的参数ID，用于生成新的参数ID
            cursor.execute("""
...
def create_attachment_tab_from_template(product_id, attachment_type, selected_pipe_codes, viewer_instance):
    """
    从模板创建新的管口附件tab页（用于从空模板创建，复制模板库中的参数结构）
    :param product_id: 产品ID
    :param attachment_type: 附件类型（如：接管法兰配对法兰、接管拉筋、防冲挡板、破涡器）
...
def _setup_tab_bar(tab_widget, viewer_instance):
    """设置tabBar属性（滚动按钮、右键菜单等）"""
    bar = tab_widget.tabBar()
    bar.setUsesScrollButtons(True)  # 启用滚动按钮（当tab过多时）
    bar.setExpanding(False)  # 不拉伸tab宽度
    bar.setElideMode(Qt.ElideNone)  # 不省略tab文字
    bar.setContextMenuPolicy(Qt.CustomContextMenu)  # 启用右键菜单
...
def _calculate_pipe_code_options(product_id, attachment_type, tab_classification, guankou_codes):
    """计算管口号的可选项（总可选项减去其他tab已选，但保留当前tab已选）"""
...
def _setup_table_header_style(table):
    """设置表头样式（背景色、边框、居中、固定高度）"""
    header = table.horizontalHeader()
    for i in range(table.columnCount()):
        header.setSectionResizeMode(i, QHeaderView.Stretch)  # 各列等宽拉伸
