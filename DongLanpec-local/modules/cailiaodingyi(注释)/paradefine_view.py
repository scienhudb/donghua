# 这个文件是材料定义页面的主入口，主要负责把 PyQt 界面、材料库查询和产品活动库写入串起来。
import os
import re
import sys
import traceback
...
# 下面这些导入分成三类：基础库、PyQt 控件，以及项目里已经封装好的数据/渲染函数。
import pymysql
...
# 收到产品管理模块发来的产品 ID 后，先存在本文件里，后面的材料查询都围绕这个产品展开。
...
# 从模板库里取管口附件参数，再按当前产品已有的附件选择写入产品活动库。
def load_pipe_attachment_from_template(product_id, template_name, force_reload=False):
    ...
    """
    # 把按钮、表格或 tab 的信号接到处理函数上，用户操作后能自动触发刷新。
    try:
        import pymysql
        import time
        ...
        if not force_reload:
            ...
            try:
                # 创建游标后集中执行 SQL，这样读取结果和提交事务都比较清楚。
                with conn_check.cursor() as cursor:
                    # 先查 产品设计活动表_管口附件附加参数表 里有没有已有记录，有数据时就能避免重复写入。
                    cursor.execute("""
                        SELECT COUNT(*) FROM 产品设计活动表_管口附件附加参数表
                        WHERE 产品ID = %s
                    """, (product_id,))
                    # 把 SQL 结果取出来，下一步会按字段名或行号继续拆解。
                    cnt = cursor.fetchone()
        ...
        try:
            ...
            # 先清理 产品设计活动表_管口附件附加参数表 里的旧记录，避免重新加载模板时重复叠加。
            if force_reload:
                # 先清理 产品设计活动表_管口附件附加参数表 里的旧记录，避免重新加载模板时重复叠加。
                cur.execute("""
            ...
            try:
                ...
                # 把整理好的参数写入 产品设计活动表_管口附件附加参数表，让产品活动库保存这次界面结果。
                tab_id_counter = 0
                insert_count = 0
        ...
        # 异常时打印完整堆栈，方便后面定位是哪一步数据或界面出了问题。
        print(f"[管口附件][模板加载] 失败: {e}")
        import traceback
        traceback.print_exc()


# 这个窗口把左侧元件列表、右侧参数表、管口材料 tab 和批量替换能力串在一起。
class DesignParameterDefineInputerViewer(QWidget):
    # 下面几组常量限定了批量替换能改哪些材料字段，以及哪些“覆层”字段要统一识别。
    ...
    # 窗口创建时先加载 UI、记录产品信息，再准备后面刷新列表和参数表用到的状态。
    ...
    # 检查用户选出的材料类型、牌号、供货状态和标准能不能在材料库里匹配到。
    ...
    # 刷新下拉框时尽量保留用户当前输入，避免联动更新把正在编辑的内容清掉。
    def _set_combo_items_keep_text(self, combo, items, keep_text=""):
        # 把候选项写入下拉框，用户选择时就能直接看到可用材料值。
    ...
    # 把可替换的材料参数按来源和元件分组，后面弹窗可以一组一组处理。
    ...
    def iter_replaceable_material_rows_for_selected_ids(self, selected_ids):
        ...
        try:
            ...
            with conn.cursor(pymysql.cursors.DictCursor) as cur:
                ...
                # =========================================================
                # 从 产品设计活动表_元件附加参数表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
                ...
                # 从 产品设计活动表_管口附加参数表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
                if need_guankou:
                    # 从 产品设计活动表_管口附加参数表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
                    cur.execute("""
                ...
                # 从 产品设计活动表_管口附件附加参数表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
                if need_attachment:
                    # 从 产品设计活动表_管口附件附加参数表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
                    cur.execute("""
    ...
    # 给材料参数生成稳定的分组键，避免不同元件或不同数据源被误合并。
    ...
    # 普通元件替换完成后，把新材料同步到左侧材料表，管口等特殊项单独处理。
    ...
    # 判断这个元件能不能同步左侧材料表，避免管口、接地装置等特殊数据被误写。
    ...
    # 从当前选中项里整理出可替换字段，决定批量替换弹窗能选哪些参数。
    ...
    def toggle_batch_replace_row(self, row):
        ...
        # 最终出现“点了但一个都选不上”的现象。
        # 同一次鼠标点击可能触发两个 Qt 信号，这里用时间戳去掉重复 toggle。
        try:
            import time
            now = time.monotonic()
        ...
        # 垫片不参与材料批量替换，这里只给用户提示，不把这一行加入选择集合。
    ...
    # 按批量替换选中集合给左侧行上色，让用户能看清哪些元件会被处理。
    ...
    # 把一组材料参数整理成字典，便于后面按材料类型、牌号等字段联动过滤。
    ...
    # 从当前可替换材料里提取旧值候选，供用户限定“只替换哪些旧材料”。
    ...
    # 弹出批量替换窗口，按用户设置的旧值条件和新值组合批量改材料参数。
    def batch_replace_selected_elements(self):
        # 这里准备下拉选择控件，材料字段会通过候选项来限制输入范围。
        ...
        try:
            ...
            # 处理 ctx match old filters 这一段逻辑。
            ...
            # 收集 old options for field，给筛选、校验或批量处理使用。
    ...
    # 读取当前分组已有的材料属性，和用户输入的新值一起组成校验上下文。
    ...
    # 统一拦截部分 Qt 事件，比如回车跳转、点击选择和批量替换模式下的特殊交互。
    def eventFilter(self, obj, event):
        """
        仅针对“模板选用”下拉框，屏蔽鼠标滚轮事件，防止滚轮误切换模板。
        其他控件/事件全部走父类默认逻辑。
        """
        # 把刷新动作延后到 Qt 当前事件结束后执行，避免读到还没稳定的界面状态。
        try:
            # 把刷新动作延后到 Qt 当前事件结束后执行，避免读到还没稳定的界面状态。
    ...
    def init_widgets(self):
        ...
        # 从 UI 文件里按对象名取控件，后面统一设置表格、按钮和输入框。
    ...
    # 左侧元件被点击时，根据当前模式刷新右侧详情或切换批量替换选中状态。
        ...
        if is_shift:
            ...
            if table and row is not None and 0 <= row < table.rowCount():
                ...
                def _apply_after_qt_shift():
                    ...
                    # 处理 invoke after selection 这一段逻辑。
    ...
    # 主 tab 切换时保留一个统一入口，后面有需要可以在这里补刷新逻辑。
    def on_tab_changed(self, index):
        self.guankou_material_category.setCurrentIndex(0)

    # 判断管口行是否适合参与批量替换，避免空行或受保护分类被改。
    ...
    # 确保默认管口材料 tab 有 Tab_ID，这样后面切换、写库和删除都有依据。
    ...
    # 管口材料 tab 切换时，按 Tab_ID 重新加载这一页的参数。
    ...
    # 复制当前管口材料 tab 的表格结构，新分类先不复用旧管口号。
    ...
    # 右键管口 tab 时弹出操作菜单，集中处理新增、删除和重命名。
    ...
    # 删除管口材料分类时，同时把数据库里对应 Tab_ID 的参数清掉。
    ...
    def show_floating_table(self, tab_index: int):
        ...
        # 处理 on parent page destroyed 这一段逻辑。
    ...
    # 统一弹出普通提示，避免每个地方重复写 QMessageBox 配置。
    ...
    def _add_enlarge_button_to_tab(self, tab_index: int):
        ...
        # 把放大按钮做成小图标按钮，既能点开参数表，又不会挤占 tab 标题。
        ...
        # 连接点击事件
        # 响应 enlarge clicked 事件，并把界面切到对应状态。
    ...
    # 根据数据库里的分类数据重建管口材料 tab，已有 tab 则尽量刷新复用。
    def build_or_refresh_guankou_tabs_from_db(self, param_map: dict):
        ...
        if "管口材料分类-壳程" not in self.guankou_tab_id_map:
            ...
            try:
                # 创建游标后集中执行 SQL，这样读取结果和提交事务都比较清楚。
                with connection.cursor() as cursor:
                    # 把界面里的新值更新到 产品设计活动表_管口附加参数表，保持数据库和页面一致。
                    cursor.execute("""
    ...
    # 进入页面时加载模板、元件、管口和参数数据，完成第一次界面渲染。
    def load_original_data(self):
        ...
        if not getattr(self, "_template_signal_connected", False):
            # 处理 update lineEdit enabled 这一段逻辑。
    ...
    # 把元件基础信息渲染到左侧列表，并把元件 ID 和图片路径挂到行数据上。
    def render_data_to_table(self, element_original_info):
        ...
        # 把整理好的业务数据写进表格单元格，界面展示和后续点击都依赖这些单元格。
        ...
        for row_index, row_data in enumerate(element_original_info):
            ...
            for col_idx, key in enumerate(headers):
                # 把普通文本包装成表格单元格，同时可以顺手设置对齐、颜色和附加数据。
                if key == "序号":
                    # 把普通文本包装成表格单元格，同时可以顺手设置对齐、颜色和附加数据。
    ...
    def _get_component_ids_for_filter(self, cursor, product_id, component_name: str):
        ...
        # 从 产品设计活动表_元件附加参数合并表 里读取当前步骤需要的数据，再交给下面整理成界面可用的格式。
    ...
    # 在数据库和当前 UI 数据里一起搜索关键字，刷新左侧元件列表。
    def filter_table_globally(self, keyword):
        ...
        try:
            ...
            # 3. UI 行 → 元件ID
            # =====================================================
            # 处理 get component ids for row 这一段逻辑。
            ...
            # 4. 数据库侧命中判断
            # =====================================================
            # 处理 keyword hits in component ids 这一段逻辑。
    ...
    def update_replaceable_material_row_value(self, item, new_value):
        ...
        try:
            ...
            with conn.cursor() as cur:
                # ---------- 1. 普通元件 ----------
                # 把界面里的新值更新到 产品设计活动表_元件附加参数表，保持数据库和页面一致。
                if source == "normal":
                    # 把界面里的新值更新到 产品设计活动表_元件附加参数表，保持数据库和页面一致。
                ...
                # ---------- 2. 合并元件 ----------
                # 把界面里的新值更新到 产品设计活动表_元件附加参数合并表，保持数据库和页面一致。
                elif source == "merged":
                    # 把界面里的新值更新到 产品设计活动表_元件附加参数合并表，保持数据库和页面一致。
                ...
                # ---------- 4. 管口附件 ----------
                # 把界面里的新值更新到 产品设计活动表_管口附件附加参数表，保持数据库和页面一致。
    ...
    # 左侧切换元件时刷新零件图片，必要时切到管口附件或特殊元件页面。
